...
void SckCharger::setup(SckBase *base)
{
	...
	// Set charger timer based on battery capacity and current limit
	int8_t timeNeeded = ceil((float)base->config.battConf.battCapacity / (float)base->config.battConf.chargeCurrent);
	chargeTimer(timeNeeded);
